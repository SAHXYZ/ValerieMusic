...
from ValerieMusic.utils.database import add_sudo, remove_sudo
from ValerieMusic.utils.decorators.language import language
from ValerieMusic.utils.extraction import extract_user
...
async def sudoers_list(client, message: Message):
    ...
    await message.reply_photo(photo="https://files.catbox.moe/x0a7zy.jpg", caption="<b>» ᴄʜᴇᴄᴋ sᴜᴅᴏ ʟɪsᴛ ʙʏ ɢɪᴠᴇɴ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ.<b/>\n\n<b>» ɴᴏᴛᴇ:<b/>  ᴏɴʟʏ sᴜᴅᴏ ᴜsᴇʀs ᴄᴀɴ ᴠɪᴇᴡ. ", reply_markup=reply_markups)
...
async def check_sudo_list(client, callback_query: CallbackQuery):
    ...
    if callback_query.from_user.id not in SUDOERS:
        return await callback_query.answer("Sᴜᴅᴏ Lɪsᴛ Dᴇᴋʜɴᴀ Hᴀɪ Mᴇʀᴇ Bᴇᴛᴇ Kᴏ 🤣", show_alert=True)
    else:
        user = await app.get_users(OWNER_ID)

        user_mention = (user.first_name if not user.mention else user.mention)
        caption = f"<b>˹ʟɪsᴛ ᴏғ ʙᴏᴛ ᴍᴏᴅᴇʀᴀᴛᴏʀs˼<b/>\n\n<b>🌹Oᴡɴᴇʀ<b/> ➥ {user_mention}\n\n"
        ...
        for user_id in SUDOERS:
            if user_id != OWNER_ID:
                try:
                    ...
                    user_mention = user.mention if user else f"<b>🎁 Sᴜᴅᴏ {count} ɪᴅ:<b/> {user_id}"
