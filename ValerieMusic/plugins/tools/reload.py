...
from ValerieMusic.utils.database import get_assistant, get_authuser_names, get_cmode
from ValerieMusic.utils.decorators import ActualAdminCB, AdminActual, language
from ValerieMusic.utils.formatters import alpha_to_int, get_readable_time
...
async def reload_admin_cache(client, message: Message, _):
    try:
        # FIX: was seeded with {} then compared `{} > time.time()` →
        # TypeError. Store int timestamp only (0 = no cooldown), guard type.
        saved = rel.get(message.chat.id, 0)
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Valerie.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await Valerie.stop_stream_force(chat_id)
        except:
            pass
