...
from ValerieMusic.utils.inline import botplaylist_markup
...
def PlayWrapper(command):
    async def wrapper(client, message):
        ...
        if message.command[0][0] == "v":
            ...
            # FIX: message.text is None for media-caption commands → crash
            # "argument of type 'NoneType' is not iterable". Fall back to
            # caption, and guard the command[0][1] index.
            text = message.text or message.caption or ""
            if "-v" in text:
                video = True
            else:
                cmd0 = message.command[0]
                video = True if len(cmd0) > 1 and cmd0[1] == "v" else None
